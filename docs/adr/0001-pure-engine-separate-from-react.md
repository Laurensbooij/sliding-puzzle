...
All board rules — shuffling, move legality, solved detection — live in a
...
- **Rules inside hooks** (`useGame` owning the logic, tested via `renderHook`). Rejected:
  shuffling and move legality need thousands of seeded runs to verify, which is fast against
